...
const verifyStripePayment = async (appointmentId) => {
    try {
        ...
        if (data.success) {
            ...
            // Reload appointments so payment becomes true
            getUserAppointments();
...
useEffect(() => {
    ...
    if (
        ...
        verifyStripePayment(appointmentId);
    ...
    <div>
      {appointments.slice(0,3).map((item,index)=>(
          ...
          <div className='flex-1 text-sm text-zinc-600'>
           ...
           <p className='text-xs mt-1'><span className='text-sm text-neutral-700  font-medium'>Date & Time</span> {slotDataFormat(item.slotDate)} | {item.slotTime} </p>   
          </div>
           <div></div>
           <div className='flex flex-col gap-2 justify-end'>
            {!item.cancelled && item.payment && !item.isCompleted && <button className='sm:min-w-48 py-2 border-rounded text-stone-500 bg-indigo-50'>Paid</button> }
           {!item.cancelled &&  !item.payment && !item.isCompleted &&<button  onClick={() => paymentStripe(item._id)}
